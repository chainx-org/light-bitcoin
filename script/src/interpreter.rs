...
fn verify_signature(
    ...
    checker.verify_signature(&signature.into(), &public, &message)
}
...
fn is_defined_hashtype_signature(version: SignatureVersion, sig: &[u8]) -> bool {
    ...
    Sighash::is_defined(version, u32::from(sig[sig.len() - 1]))
}
...
fn verify_witness_program(
    ...
    let (mut stack, script_pubkey): (Stack<_>, Script) = match witness_program.len() {
        32 => {
            ...
            if script_pubkey_hash != H256::from_slice(&witness_program[0..32]) {
        ...
        match opcode {
            ...
            Opcode::OP_CHECKSEQUENCEVERIFY => {
                if flags.verify_checksequence {
                    ...
                    if (sequence & (i64::from(SEQUENCE_LOCKTIME_DISABLE_FLAG)).into()).is_zero()
            ...
            Opcode::OP_RIPEMD160 => {
                ...
                stack.push(v.as_bytes().to_vec().into());
            }
            Opcode::OP_SHA1 => {
                ...
                if flags.verify_checkdatasig =>
            {
                ...
                let success = verify_signature(checker, signature, pubkey.into(), message_hash);
                match opcode {
...
mod tests {
    ...
    fn basic_test_with_flags(
        script: &Script,
        flags: &VerificationFlags,
        expected: Result<bool, Error>,
        expected_stack: Stack<Bytes>,
    ) {
        let checker = NoopSignatureChecker;
        let version = SignatureVersion::Base;
        let mut stack = Stack::new();
        assert_eq!(
            eval_script(&mut stack, script, &flags, &checker, version),
            expected
        );
        if expected.is_ok() {
    ...
    fn test_hash256() {
        ...
        let result = Ok(true);
        let stack =
            vec!["9595c9df90075148eb06860365df33584b75bff782a510c6cd4883a419833d50".into()].into();
        basic_test(&script, result, stack);
    }
    ...
    fn test_sha256() {
        ...
        let result = Ok(true);
        let stack =
            vec!["2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824".into()].into();
        basic_test(&script, result, stack);
    }
        ...
        assert_eq!(
            ...
            run_witness_test_tx_test("51".into(), &tx, &flags, 1000, 0)
                .and_then(|_| run_witness_test_tx_test(
                    "60144c9c3dfac4207d5d8cb89df5722cb3d712385e3f".into(),
                    &tx,
                    &flags,
                    2000,
                    1
    ...
    fn witness_unknown_program0_lengh() {
        ...
        assert_eq!(
            ...
            run_witness_test_tx_test("51".into(), &tx, &flags, 1000, 0)
                .and_then(|_| run_witness_test_tx_test(
                    "00154c9c3dfac4207d5d8cb89df5722cb3d712385e3fff".into(),
                    &tx,
                    &flags,
                    2000,
                    1
    ...
    fn witness_single_anyone_same_index_value_changed() {
        ...
        assert_eq!(
            ...
            run_witness_test_tx_test("51".into(), &tx, &flags, 1000, 0)
                .and_then(|_| run_witness_test_tx_test(
                    "00144c9c3dfac4207d5d8cb89df5722cb3d712385e3f".into(),
                    &tx,
                    &flags,
                    2000,
                    1
    ...
    fn witness_valid_p2sh_p2wpkh() {
        ...
        assert_eq!(
            Ok(()),
            run_witness_test_tx_test(
                "a914fe9c7dacc9fcfbf7e3b7d5ad06aa2b28c5a7b7e387".into(),
                &tx,
                &flags,
                1000,
                0
    ...
    fn witness_valid_p2sh_p2wsh() {
        ...
        assert_eq!(
            Ok(()),
            run_witness_test_tx_test(
                "a9142135ab4f0981830311e35600eebc7376dce3a91487".into(),
                &tx,
                &flags,
                1000,
                0
    ...
    fn witness_mixed_transaction() {
        ...
        assert_eq!(
            ...
            .and_then(|_| run_witness_test_tx_test(
                "76a9144c9c3dfac4207d5d8cb89df5722cb3d712385e3f88ac".into(),
                &tx,
                &flags,
                1002,
                2
    ...
    fn witness_single_output_oob() {
        ...
        assert_eq!(
            Ok(()),
            run_witness_test_tx_test("51".into(), &tx, &flags, 1000, 0).and_then(|_| {
                run_witness_test_tx_test(
    ...
    fn witness_1_byte_push_not_witness_script_pubkey() {
        ...
        assert_eq!(
            Ok(()),
            run_witness_test_tx_test("600101".into(), &tx, &flags, 1000, 0)
        );
    }
    ...
    fn witness_version_must_use_op1_to_op16() {
        ...
        assert_eq!(
            Ok(()),
            run_witness_test_tx_test("0110020001".into(), &tx, &flags, 1000, 0)
        );
    }
    ...
    fn witness_program_push_must_be_canonical() {
        ...
        assert_eq!(
            Ok(()),
            run_witness_test_tx_test("604c020001".into(), &tx, &flags, 1000, 0)
        );
    }
    ...
    fn witness_bip143_example4() {
        ...
        assert_eq!(
            Ok(()),
            run_witness_test_tx_test(
                "a9149993a429037b5d912407a71c252019287b8d27a587".into(),
                &tx,
                &flags,
                987654321,
                0
    ...
    fn test_num2bin_all() {
        ...
        test_num2bin_num(
            256.into(),
            1.into(),
            Err(Error::ImpossibleEncoding),
            vec![0x00],
        );
        test_num2bin_num(
            1.into(),
            (MAX_SCRIPT_ELEMENT_SIZE + 1).into(),
            Err(Error::PushSize),
            vec![0x00],
        );
        ...
        test_num2bin_num(
            ...
            Ok(true),
            iter::once(0x2a)
                .chain(iter::repeat(0x00).take(518))
    ...
    fn checkdatasig_spec_tests() {
        ...
        basic_test_with_flags(
            &correct_signature_script,
            &incorrect_flags,
            Err(Error::DisabledOpcode(Opcode::OP_CHECKDATASIG)),
            vec![].into(),
        );
        ...
        let incorrectly_encoded_signature_script = Builder::default()
            ...
            .into_script();
        basic_test_with_flags(
            &incorrectly_encoded_signature_script,
            &correct_flags,
    ...
    fn checkdatasigverify_spec_tests() {
        ...
        basic_test_with_flags(
            &correct_signature_script,
            &incorrect_flags,
            Err(Error::DisabledOpcode(Opcode::OP_CHECKDATASIGVERIFY)),
            vec![].into(),
        );
