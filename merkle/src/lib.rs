...
impl Deserializable for PartialMerkleTree {
    ...
    where
        ...
        T: io::Read,
    {
        let tx_count = reader.read()?;
        let hashes = reader.read_list()?;
...
mod tests {
    ...
    use light_bitcoin_primitives::{h256_rev, H256};
    ...
    impl PartialMerkleTree {
        ...
        fn damage(&mut self, rng: &mut ThreadRng) {
            ...
            hash.as_bytes_mut()[(bit >> 3) as usize] ^= 1 << (bit & 7);
            hashes[n] = hash;
        }
    }
        ...
        for tx_count in tx_counts {
            ...
            for att in 1..15 {
                ...
                let pmt2: PartialMerkleTree =
                    deserialize(serialized.as_ref()).expect("Could not deserialize own data");
                ...
                assert_ne!(merkle_root_2, H256::default());
                ...
                for _ in 0..4 {
                    let mut pmt3: PartialMerkleTree = deserialize(serialized.as_ref()).unwrap();
                    pmt3.damage(&mut rng);
    ...
    fn merkle_block_construct_from_txids_found() {
        ...
        assert_eq!(merkle_root, block.header.merkle_root_hash);
